...
class LineR3:
    def __init__(self, origin=None, point=None, line=None):
        if line is None:
            self.origin = np.random.randint(-5, 5, size=(3,)) if origin is None else np.array(origin, dtype=np.float64)
            self.point = np.random.randint(-5, 5, size=(3,)) if point is None else np.array(point, dtype=np.float64)
